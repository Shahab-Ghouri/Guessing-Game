...
class ImranAbbas{
	...
	public:
		bool check_pers(){
			char response[10];
			ifstream fileptr;
			fileptr.open("MALE/imranabbas.txt");
			if(fileptr){
				getline(fileptr,ans);
				cout<<" IS Your Character from "<<ans<<" ? (YES/NO) : ";
				string phrase = "IS Your Character from "+ans; 
				...
				cout<<" Does Your Character Has Played in a "<<ans<<" movie? (YES/NO) : ";
				phrase = "Does Your Character Has Played in a "+ans+" movie"; 
		...
		bool check_in_file(LinkedList &obj,int dom,int size, bool flag){
			...
			file.open("imranabbas.txt",ios::in);
			temp = obj.head;
